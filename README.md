...
Рядом с папкой ```./src``` лежит файл ```index.html``` с определением ОС пользователя по пользовательскому агенту User Agent и перенаправлением (редиректом) на соответствующую версию приложения. Для iOS - это файл ```.reality```, который лежит там же. Оба эти файла надо положит в корень сайта рядом с папкой ```./app``` веб-приложения.